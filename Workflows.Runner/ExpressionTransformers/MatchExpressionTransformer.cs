...
namespace Workflows.Runner.ExpressionTransformers
{
    ...
    internal class MatchExpressionTransformer
    {
        ...
        public MatchTransformationResult TransformCommandMatch(LambdaExpression matchExpression, WorkflowContainer workflowInstance)
        {
            if (matchExpression == null)
                throw new ArgumentNullException(nameof(matchExpression));
            // Step 3: Normalize the original expression ONLY for the Runner (Tier 3 execution)
            var matchExpressionNormalized = new MatchExpressionNormalizer().NormalizeCommandMatch(matchExpression, workflowInstance);
            var normalizedExpression = matchExpressionNormalized.NormalizedExpression;
